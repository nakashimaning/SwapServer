	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
	    ...
	    try {
	        ...
	        if ("product".equals(paths[1])) {
	            // 獲取市集商品，根據 userId 是否存在決定顯示全部或排除指定 userId
	            if (paths.length == 2) {
	            ...
	            // 查詢產品詳細信息
	                ...
	                Integer productId = Integer.parseInt(productIdParam);
	                Integer currentUserId = req.getParameter("currentUserId") != null
	                        ? Integer.parseInt(req.getParameter("currentUserId"))
	                        : null;
...
//					String productIdParam = req.getParameter("productId");
//					String userIdParam = req.getParameter("userId");
//					if (productIdParam == null || productIdParam.isEmpty() || userIdParam == null
//							|| userIdParam.isEmpty()) {
//						resp.sendError(HttpServletResponse.SC_BAD_REQUEST, "Missing or empty parameters");
//						return;
//					}
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		if (pathInfo != null && pathInfo.equals("/product/favorite")) {
			...
			try {
				...
				boolean isApplied = marketProductService.applyForExchange(userId, toBeTradedProductId,
						applyingProductId);
